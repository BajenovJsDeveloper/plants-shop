...
export function Cart({ navigation }) {
  ...
  return (
    <MainLayout>
      <Heading size="md" textAlign="center" color="teal.700">Your Order</Heading>
      {
        cartList.map(item => {
        return (
          <CartItem
  ...
  return (
    <SlideFade in={slide} duration={300}>
      <Pressable p={2} rounded="md" mt={4} bg="white" key={item.item.id}> 
        <Heading p={2} size="sm" bg="teal.600" color="white" borderRadius={6}>{item.item.title}</Heading>
        <HStack p={4}>
...
const ItemDescription = ({ count, price }) => {
  return (
    <>
      <HStack>
        <Text>Quantity:</Text>
        <Text bg="teal.500" px={2} ml={4} rounded="md" fontSize={16} color="white">{count}</Text>
      </HStack> 
      <Text>Price: ${price}</Text>
      <Text fontWeight="bold" fontSize="md">Total: $ 
      {countTotalPrice(price, count)}
      </Text>
    </>
  )
...
const OrderView = ({ total, disabled }) => {
  return (
    <Box rounded="md" p={4} mt={4} bg="white">
      {disabled 
        ? <Text textAlign="center" fontSize="lg" color="gray.600">Cart is empty.</Text>
        : <HStack alignItems="center">
            <Text fontSize={16} color="gray.600">Total for Pay: </Text>
            <Text fontSize={18} fontWeight="bold" color="teal.500" ml={4}>$ {total}</Text>
          </HStack>
      }
